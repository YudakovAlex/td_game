	...
	for i := 0; i < route.point_count-1; i += 1 {
		...
		if ax == bx {
			...
			for y := min_y; y <= max_y; y += 1 {
				if ax >= 0 && ax < MAP_W && y >= 0 && y < MAP_H { g.tiles[y][ax].kind = .Path }
			}
		} else if ay == by {
			min_x, max_x := ax, bx
			if min_x > max_x { min_x, max_x = max_x, min_x }
			for x := min_x; x <= max_x; x += 1 {
				if x >= 0 && x < MAP_W && ay >= 0 && ay < MAP_H { g.tiles[ay][x].kind = .Path }
			}
		}
	}
...
init_map :: proc(g: ^Game) {
	for y := 0; y < MAP_H; y += 1 {
		for x := 0; x < MAP_W; x += 1 { g.tiles[y][x].kind = .Buildable }
	}
...
draw_path :: proc(g: ^Game) {
	...
	for route_index := 0; route_index < level.route_count; route_index += 1 {
		...
		for i := 0; i < route.point_count-1; i += 1 {
			rl.DrawLineEx(route.points[i],route.points[i+1],5,rl.BROWN)
		}
	}
}
